"""Command-line entry point: ``python -m orchestrant.smoke``.

Runs the wheel smoke checks (see :mod:`orchestrant.smoke.checks`) and prints
...
from orchestrant.smoke.checks import run_all


if TYPE_CHECKING:
    from orchestrant.smoke.checks import CheckResult
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m orchestrant.smoke",
        description="Exercise the installed ML wheels (torch, torchvision, "
        "onnxruntime, onnxruntime-genai, opencv, pillow, pyav, tvm, litert) "
        "with real work.",
    )
    parser.add_argument(
        "--json",
